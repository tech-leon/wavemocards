...
### Line Height（行高）
每個 `.type-*` class 都自帶 `line-height`（不只 `font-size`），閱讀節奏是字級系統的一部分,不再由各元件臨時補。內文放寬以利三語 CJK 閱讀,展示字與按鈕保持緊湊:內文 `.type-body` / `.type-body-lg` 1.7、`.type-body-sm` 1.6、`.type-caption` 1.5、`h1`–`h3` 與標題類 1.3、`.type-hero-cta` 1.2、`.type-card-display` / `.type-modal-display` 1.15、`.type-hero-display` 1.05、`.type-button` / `.type-button-lg` 1。不要在元件內另設 line-height;某 role 要不同值就改 `globals.css` 的 class。
...
| Sticky chrome | `shadow-soft` + `backdrop-blur-sm` | 固定頂部的 Header |
| Floating control | `shadow-float`（回到頂部）/ `shadow`（卡片小鈕） | 回到頂部按鈕；浮在卡片上的小圓鈕（情緒卡加減鈕） |
| Popover | `shadow-lg` | dropdown 與 popover 層 |

`shadow-soft` 與 `shadow-float` 是帶品牌青調、低透明度、分層的 elevation token（定義於 `globals.css`），讓「浮起」的表面輕柔抬升,呼應退潮平靜海面的調性,而非投下生硬的中性灰陰影。Shadcn 內部層（dropdown、drawer）維持預設 `shadow-md` / `shadow-lg`。
...
### Navigation
- **Header（`src/components/layout/Header.tsx`）：** sticky `top-0 z-50`、`bg-muted` + `shadow-soft` + `backdrop-blur-sm`，`max-w-7xl` 容器。桌機三欄 grid（nav｜logo｜actions），行動版 logo + ThemeToggle + MobileNav。
...
- **尊重 reduced motion。** `<MotionConfig reducedMotion="user">`（在 `ThemeProvider.tsx`）讓所有 framer-motion 動畫遵循 OS「減少動態效果」設定 — 去掉位移/縮放,保留 opacity。`globals.css` 的 `@media (prefers-reduced-motion: reduce)` 區塊則把 CSS transition/animation 與平滑捲動近乎即時收掉。新動畫自動繼承,不要繞過。
...
誠實列出系統尚未完全解決的部分（對齊 reference 設計文件的「Known Gaps」慣例）。

- **`text-muted-foreground` 在頁面底色上是臨界值。** Light 的 muted-foreground 為 gray-600（`#6c757d`），在白卡與 `bg-muted` 上是 4.69:1,直接放在 `bg-background` 上約 4.45:1 — 實質 AA,在最亮表面上微微低於 4.5。刻意維持 gray-600（它是正確的自家 ramp 階;自訂一個脫離 ramp 的 hex 會違反 One-Gray Rule）。除非 audit 要求,否則不再加深。
- **ProgressBar 未完成步數字屬裝飾性對比。** `gray-450` 數字在 `bg-background` 上約 1.58:1。它標示尚未抵達的步驟（狀態另由位置與 active 圓點承載），因此視為非必要裝飾而非可讀文字。若哪天步數字需要單獨被讀,再重新處理。
- **`hover:text-main` 連結 hover 態低於 AA。** 內文連結靜止狀態為 `text-foreground` / `text-muted-foreground`（可讀），只有 hover 時轉 `main`（`#3C9DAE`,約 3.01:1）。hover 染色是瞬時 affordance、非閱讀狀態,故保留原樣;若日後出現持續性的 body 尺寸 `main` 連結,改用 `main-dark`。
- **Letter-spacing 尚未 token 化。** 行高已是每個 type role 的一部分,但字距仍維持字型預設。CJK 不適合負字距;只有 EN locale 的 `.type-hero-display` 是小幅負值的候選,尚未加入。
- **情緒卡名刻意維持品牌青、接受低對比。** 粉彩底上的情緒卡名與分類名用 `.type-subsection-title`（品牌 `main` 青），每個分類都不過 WCAG AA（1.6–2.5:1）。這是刻意保留:卡片、標題、動作一致的青色嗓音是品牌色調統一的核心,在這些裝飾性標籤上優先於對比目標。注意 `text-*` utility 無法改色（未分層的 `.type-subsection-title { color: main }` 會贏過任何 utility）;若日後要無障礙版本,需另開 sibling type-class 用固定深灰（如 `gray-800`,可過 AA 5.9–9.2:1）—— 隨時可加,代價是失去品牌青調。
- **元件／整合視覺回歸靠人工。** 無自動視覺 diff;字級、對比、暗色模式一致性皆由瀏覽器人工驗證（見專案驗證流程）。
