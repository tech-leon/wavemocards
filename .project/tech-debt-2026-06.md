...
7. 加 `vitest`(node 環境、`@` alias、無 mock),測 6 個純函式模組共 78 case:`records-validation`(信任邊界)、`i18n/locale`、`records-query`、`record-utils`、`records-search`(純 tokenizer)、`profile-insert`(守 #2 欄位漂移)。`pnpm test` 綠燈,反向 mutation 驗證測試確實咬合。
