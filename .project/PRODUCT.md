...
> 本文件供設計與開發決策參考。若與實際產品方向不符，請直接修改。
> Updated: 2026-08-08

## Register

稱呼使用者用「你」，不用「您」；使用者自己的內容用第一人稱（我的紀錄、我的情緒故事）。語氣像陪著走一段的朋友，不是治療師也不是系統通知。句子短、口語，避免臨床與診斷詞彙（症狀、療程、障礙、評估）。引導步驟用邀請句「想一想那時候發生了什麼」，不用命令句「請輸入內容」。不使用驚嘆號堆疊與稱讚式感嘆（太棒了、做得好），完成頁是致意不是慶祝。錯誤訊息只講發生什麼事與下一步，不歸咎使用者。三語語氣一致：日文用です・ます体但不堆敬語，英文維持相同的第二人稱與平實句型。
...
Wave Emotion Cards（浪潮情緒卡）用九大類情緒卡引導使用者辨識並命名情緒，記錄事件脈絡（故事、反應、結果、感受、期待、行動）與前後情緒強度（1 到 5），並提供紀錄回顧、全文搜尋與統計分析，幫助使用者看見自己的情緒模式。
