...
CREATE OR REPLACE FUNCTION public.search_emotion_records_paginated(
  ...
  p_keyword_tokens text[] DEFAULT NULL,
  ...
  WITH filtered_records AS (
    ...
    WHERE er.user_id = p_user_id
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1
          FROM unnest(coalesce(p_keyword_tokens, ARRAY[]::text[])) AS token
          WHERE token <> ''
            AND (
              unaccent(coalesce(er.story, '')) ILIKE '%' || unaccent(token) || '%'
              OR unaccent(coalesce(er.reaction, '')) ILIKE '%' || unaccent(token) || '%'
              OR unaccent(coalesce(er.results, '')) ILIKE '%' || unaccent(token) || '%'
              OR unaccent(coalesce(er.feelings, '')) ILIKE '%' || unaccent(token) || '%'
              OR unaccent(coalesce(er.expect, '')) ILIKE '%' || unaccent(token) || '%'
              OR unaccent(coalesce(er.actions, '')) ILIKE '%' || unaccent(token) || '%'
            )
        )
        OR (
