...
export function MobileNav({ user }: MobileNavProps) {
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMenu}
        className="lg:hidden p-2 text-muted-foreground hover:text-main transition-colors"
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Mobile Menu */}
            <motion.div
              className="fixed top-[70px] right-0 h-full w-64 bg-background shadow-lg z-50 lg:hidden"
              role="dialog"
              aria-modal="true"
              aria-label="行動裝置導覽選單"
              initial={{ x: '100%' }}
                  ...
                  <ul className="space-y-4">
                    <li>
                      <Link
                        href="/about-emotions"
                        onClick={closeMenu}
                        className="flex items-center gap-3 text-foreground hover:text-main transition-colors"
                      >
                    ...
                    <li>
                      <form action={handleSignOut}>
                        <button
                          type="submit"
                          className="flex items-center gap-3 text-foreground hover:text-main transition-colors w-full"
                        >
